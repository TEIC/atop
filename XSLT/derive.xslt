...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="atop:pSource" as="xs:anyURI"/>
  ...
  <xsl:template match="/" name="xsl:initial-template" as="document-node()">
    <xsl:copy>
      ...
      <xsl:result-document href="/tmp/{$atop:pDebugName}_post-pass_01">
      ...
      <xsl:result-document href="/tmp/{$atop:pDebugName}_post-pass_02">
      ...
      <xsl:result-document href="/tmp/{$atop:pDebugName}_post-pass_03">
      ...
      <xsl:result-document href="/tmp/{$atop:pDebugName}_post-pass_04">
        <xsl:sequence select="$vPass04"/>
      </xsl:result-document>
      <!-- pass 04: macro deletion -->
      <xsl:variable name="vPass05" as="node()+">
        <xsl:apply-templates select="$vPass04" mode="atop:mPass04"/>
      </xsl:variable>
      <xsl:result-document href="/tmp/{$atop:pDebugName}_post-pass_05">
        <xsl:sequence select="$vPass05"/>
      </xsl:result-document>
      <!--
        Note: need a pass here that processes (content|sequence|alternate)[not(.//*)] and
        replaces its content with a <empty>, right?
      -->
      <!-- output -->
      <xsl:sequence select="$vPass05"/>
    </xsl:copy>
  ...
  <xd:doc>
    <xd:desc>If a dataRef/@key matches oneof the things-to-be-deleted,
    then do nto copy it, but rather warn user there is likely to be a
  ...
  <xsl:template match="dataRef" mode="atop:mPass04" as="node()">
    ...
    <xsl:choose expand-text="yes">
      <xsl:when test="@key = $tpDeleteUs">
        ...
        <!--
          If I do not have a sibling, then my removal leaves my parent empty.
          That is OK in a <specGrp>, and cannot happen in a <schemaSpec>. But
          in a <datatype> or inside a <content> it is likely a problem.
        -->
        <xsl:if test="not( ../*[2] ) and not( parent::specGrp )">
          <xsl:message select="'Warning: reference to '
                             ||@key
                             ||' datatype leaves an empty '
                             ||local-name(..)
                             ||' (in '
                             ||atop:unique-ident(.)
  ...
  <xd:doc>
    <xd:desc>If a macroRef/@key matches oneof the things-to-be-deleted,
      then do not copy it.</xd:desc>
    <xd:param name="tpDeleteUs">list of NCNames of the idents of elements to be deleted</xd:param>
  </xd:doc>
